...
def build_board_tokens(
    ...
    black_mask = torch.tensor(
        [board is not None and board.turn == chess.BLACK for board in boards],
        device = x.device,
        dtype  = torch.bool,
    )
...
def extract_all_tokens(
    ...
    for b, board in enumerate(boards):
        if board is None:
            ...
            candidate_moves.append(get_candidate_moves(board, logits_move[b], user_queried_ucis[b], max_moves))
